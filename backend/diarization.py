...
def main(audio_file_path="firstRecord.wav"):
    ...
    recording, sample_rate = sf.read(audio_file_path)
    recording = recording.astype(np.float32)
    recording_norm = recording / np.max(np.abs(recording))
    ...
    if malayalam_txt and "⚠" not in malayalam_txt:
        ...
        output_file_base64 = text_to_speech(malayalam_llm_response)
        return output_file_base64


    else:
        print("⚠ Transcription failed or unclear. Skipping rest of pipeline.")
        return "no_output"
